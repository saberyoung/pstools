...
def VST(_list,triggerid,enviroment,username,password,containerid,_userpriority,_airmass, _skyTransparency, _fli,_seeing,filtro,_exp,_dither,_repeat):

    # generate OB list
    _oblist = {}   
    for ii in _list.split('\n'):       
        if len(ii)==0:continue
        if ii[0]=='#':continue

        try:ra,dec,nob = float(ii.split()[0]),float(ii.split()[1]),float(ii.split()[-1])
        except:continue
        ...
        try:_oblist[nob].append([ra,dec,_time1,_time2])
        except:
            _oblist[nob]=[]
            _oblist[nob].append([ra,dec,_time1,_time2])
    ...
    for _nob in _oblist:
        ...
        _npointing = 0
        for _ra,_dec,_time1,_time2 in _oblist[_nob]:
            _npointing+=1
...
def schmidt(_list,_scheduler,filtro,rep,exp,dit,_id):
    import sys
    import numpy as np
    import pylab as pl
    import matplotlib.pyplot as plt
    import pstplot,visibility
    ...
    for ii in _list.split('\n'):
        ...
        if ii[0]=='#':continue
        try:
            ra,dec=float(ii.split()[0]),float(ii.split()[1])       
        except:continue
        ...
        for _n,_f in enumerate(filtro.split(',')):
            _save = 1 # save pic
            _expw += _exp1%(_n,_too['obs'],rep,exp,_tof[_f],_save,dit,_id)
        _expf=_exp%_expw
