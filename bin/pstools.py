...
if __name__ == "__main__":
    ...
    if _mode == 1:    # - serve alert
        ...
        pst.manschedule.man_search(args.fits, args.tel, _verbose, _log)

    elif _mode == 4:  # - configure file
        # general file: $pstool_dir/default/$pstools.par
        # telescope file: $pstool_dir/default/$tel_name.par
        if args.tel is None:
            arglist,optlist = pst.configure.load_config()
            _tell = arglist['observe']['telescope']
            if _tell is None: sys.exit('Error: option tel wrong ...')        
        else: _tell = args.tel
        _dict = {}
        for _tel in _tell.split(','): 
            _dict[_tel] = {}
            arglist,optlist = pst.configure.load_config(_tel,_verbose=_verbose,_log=_log)
            _dict[_tel]['general'],_dict[_tel]['telescope'] = arglist, optlist
        if _verbose:
            done = False
            while not done:
                answ = input('which telescope: %s or [q]uit?'%_tell.split(','))
                if answ == 'q': done=True
                elif answ in _tell.split(','):
                    print ('\n','>'*5,'for %s\n'%answ)
                    for _class in ['general','telescope']:
                        print ('\t%s params:'%_class)
                        for _key in _dict[answ][_class]:
                            print ('\t - %s :'%_key)
                            for _key0 in _dict[answ][_class][_key]:
                                print ('\t--> %s : %s'%(_key0,_dict[answ][_class][_key][_key0]))
